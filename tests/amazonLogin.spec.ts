...
test.beforeEach(async ({ page }) => {
  ...
  await page.goto(
    "/ap/signin?openid.return_to=https%3A%2F%2Fwww.amazon.in%2F%3Fref_%3Dnav_ya_signin&openid.identity=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0%2Fidentifier_select&openid.assoc_handle=inflex&openid.mode=checkid_setup&openid.claimed_id=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0%2Fidentifier_select&openid.ns=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0",
  );
...
test.describe("TC_001 - Successful Login Scenarios", () => {
  test("User should see Amazon Home page when successful login", async ({
    page,
  }) => {
...
test.describe("TC_002 - Invalid Login Scenarios", () => {
  test("User should see validation message when email is invalid", async ({
    page,
  }) => {
  ...
  test("User shoule see validation message when password is invalid", async ({
    page,
  }) => {
    const invalidPage = new AmazonLoginPage(page);
    await invalidPage.login(validEmail, invalidPwd);
    await expect(invalidPage.pwdHeaderErrormsg).toHaveText(
      "There was a problem",
    );
    await expect(invalidPage.pwdErrorMsg).toHaveText(
...
test.describe("TC_003 - Empty Field Validations", () => {
  test("User should see validation message when email is empty ", async ({
    page,
  }) => {
    const LoginEmptyPage = new AmazonLoginPage(page);
    await LoginEmptyPage.login("");
    await expect(LoginEmptyPage.enterEmailErrorMsg).toHaveText(
      "Enter your mobile number or email",
    );
  });

  test("User should see validation message when password is empty", async ({
    page,
  }) => {
    const PwdMissingAlert = new AmazonLoginPage(page);
    await PwdMissingAlert.login(validEmail, "");
    await expect(PwdMissingAlert.authoPwdMissingAlert).toHaveText(
      "Enter your password",
    );
  });
});
...
test.describe("TC_004 - Boundary Test Case of LongInput Validations", () => {
  test("User should see valiation when give email long input value", async ({
    page,
  }) => {
  ...
  test("User should see valiation when password has long input value", async ({
    page,
  }) => {
    const longInputPage = new AmazonLoginPage(page);
    await longInputPage.login(validEmail, longPwd);
    await expect(longInputPage.pwdErrorMsg).toHaveText(
      "Your password is incorrect",
    );
  });
});
test.describe("TC_005 - Boundary Test Case of Minimum Input Value Validations", () => {
  test("User should see valiation Msg when email is minimum input value", async ({
    page,
  }) => {
  ...
  test("User should see valiation Msg when pwd is minimum input value", async ({
    page,
  }) => {
    const minInputPage = new AmazonLoginPage(page);
    await minInputPage.login(validEmail, minPwd);
    await expect(minInputPage.pwdErrorMsg).toHaveText(
      "Your password is incorrect",
    );
  });
});
test.describe("TC_006 - Boundary Test Case of Maximum Input Value Validations", () => {
  test("User should see valiation Msg when email has maximum input value", async ({
    page,
  }) => {
  ...
  test("User should see valiation Msg when pwd has maximum input value", async ({
    page,
  }) => {
    const maxInputPage = new AmazonLoginPage(page);
    await maxInputPage.login(validEmail, maxPwd);
    await expect(maxInputPage.pwdErrorMsg).toHaveText(
      "Your password is incorrect",
    );
  });
});
test.describe("TC_007 - Boundary Test Case of Leading and Trailing Spaces Validations", () => {
  ...
  test("User should see valiation Msg when email have only spaces", async ({
    page,
  }) => {
    const spacesPage = new AmazonLoginPage(page);
    await spacesPage.login(onlySpacesEmail);
    await expect(spacesPage.enterEmailErrorMsg).toHaveText(
      "Enter your mobile number or email",
    );
  });

  test("User should see valiation Msg when phone has leading spaces", async ({
    page,
  }) => {
    const spacesPage = new AmazonLoginPage(page);
  ...
  test("User should see valiation Msg when phone has trailing spaces", async ({
    page,
  }) => {
    const spacesPage = new AmazonLoginPage(page);
...
test.describe("TC_008 - Login Functional Tests", () => {
  test("Login form is submitted when enter key is pressed", async ({
    page,
  }) => {
    ...
    await expect(edgeCasePage.nav_list_Hello_sign_in).not.toContainText(
      "sign in",
    );
  });
});

test.describe("TC_009 - Security Testing", () => {
  test("User should see validation msg when do SQL Injection", async ({
    page,
  }) => {
  ...
  test("User should see validation msg when do XSS attack", async ({
    page,
  }) => {
    const securityPage = new AmazonLoginPage(page);
  ...
  test("User should see validation msg when do HTML Injection", async ({
    page,
  }) => {
    const securityPage = new AmazonLoginPage(page);
...
test.describe("TC_010 - Session Managment", () => {
  test("Refresh Page after successful login", async ({ page }) => {
    ...
    await expect(sessionPage.nav_list_Hello_sign_in).not.toContainText(
      "sign in",
    );
    await page.reload();
    await expect(sessionPage.nav_list_Hello_sign_in).not.toContainText(
  ...
  test("browser back button should not reopen account page after successful logout", async ({
    page,
  }) => {
    const sessionPage = new AmazonLoginPage(page);
    await sessionPage.login(validEmail, validPwd);
    ...
    await expect(
      sessionPage.signIn_Create_Account.getByRole("heading"),
    ).toContainText("Sign in", { timeout: 15000 });
    await page.goBack({ waitUntil: "domcontentloaded" });
    await page.waitForLoadState("domcontentloaded");
    // chekcing the proctected page url able to access it
    await page.goto(
      "https://www.amazon.in/gp/css/homepage.html?ref_=nav_AccountFlyout_ya",
      { waitUntil: "domcontentloaded" },
    );
    await expect(sessionPage.nav_list_Hello_sign_in).toContainText(
      "Hello, sign in",
      { timeout: 15000 },
    );
    //  Abort all pending requests so teardown doesn't hang ───────────
